...
public enum Z1ooKeeperUtil {
    ...
    private final static org.slf4j.Logger logger = LoggerFactory.getLogger(Z1ooKeeperUtil.class);
    ...
    public void setDefaultAcl(String jsonAcl) {
        ...
        JSONObject jsonObject = JSONUtil.parseObj(jsonAcl);

        JSONArray acls = jsonObject.getJSONArray("acls");
    ...
    private void exportTreeInternal(Set<LeafBean> entries, String path, ZooKeeper zk, String authRole) throws InterruptedException, KeeperException {
        // 1. List leaves
        entries.addAll(listLeaves(zk, path));
        // 2. Process folders
    ...
    public ZKNode listNodeEntries(ZooKeeper zk, String path) throws KeeperException, InterruptedException {
            ...
            for (String child : children) {
                if (!child.equals(ZK_SYSTEM_NODE)) {
                    ...
                    if (isFolder) {
                        ...
                        leaves.add(this.getNodeValue(zk, path, childPath, child));
                    }
...
//    @Deprecated
    public List<LeafBean> listLeaves(ZooKeeper zk, String path) throws InterruptedException, KeeperException {
        List<LeafBean> leaves = new ArrayList<>();
